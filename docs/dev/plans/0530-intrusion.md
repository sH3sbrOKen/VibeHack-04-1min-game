...
- 全屏视觉变成阴森科幻风格
- 出现专属乱入消息卡
- 玩家点「收到」→ 乱入触发，endGame 时走乱入世界线，跳过原故事线判定
- 玩家点其他 / 不操作 → t=5 时乱入结束，返回原故事线正常结算
...
```js
const INTRUSIONS = {
  // 苏联女孩乱入：SCAM_TARGET + DEMO第3条MAIN触发，点收到跳SPACETIME_CHAIN
  SCAM_TARGET: {
    id: 'SPACETIME_RADIO',
    card: {
      senderName: '——',
      body: '……你能听到吗。这里是1987年列宁格勒。\n信号很弱。如果你收到了，请回复我。',
    },
...
- `#game` 加 CSS class `intrusion-active`
- 清空 `#msg-area`，渲染乱入消息卡（发信人「——」，专属 class）
- 乱入卡有独立 CSS 样式（见第6条）
...
1. 主线第15秒：全屏变暗绿色，出现「——」的消息卡
2. 点「收到」→ 立即结算，结算页出现乱入世界线「故事继续」按钮
3. 点其他按钮 / 不操作到t=5 → 视觉恢复，正常结算，无乱入按钮
4. SCAM_TARGET 第15秒同样触发，行为与主线一致
5. 乱入触发后，原故事线的世界线判定（escapeType）不影响结算页的乱入按钮
